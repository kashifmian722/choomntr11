...
class ProfileController
{
    ...
    public function save(Request $request, RequestDataBag $dataBag, MerchantEntity $merchant, SalesChannelContext $salesChannelContext): JsonResponse
    {
        ...
        if($dataBag->has('whatsapp')){
            $this->merchantRepository->update([
                [
                    'id' => $merchant->getId(),
                    "customFields" => [
                        "whatsapp" => $dataBag->get("whatsapp")
                    ]
                ]
        ...
        if($dataBag->has('radius')){
            $this->merchantRepository->update([
                [
                    'id' => $merchant->getId(),
                    "customFields" => [
                        "radius" => $dataBag->get("radius")
                    ]
                ]
        ...
        if($dataBag->has('longitude')){
            $this->merchantRepository->update([
                [
                    'id' => $merchant->getId(),
                    "customFields" => [
                        "longitude" => $dataBag->get("longitude")
                    ]
                ]
        ...
        if($dataBag->has('latitude')){
            $this->merchantRepository->update([
                [
                    'id' => $merchant->getId(),
                    "customFields" => [
                        "latitude" => $dataBag->get("latitude")
                    ]
                ]
...
if ($dataBag->has('Facebook')) {
    $this->merchantRepository->update([
        [
            'id' => $merchant->getId(),
            "customFields" => [
                "Facebook" => $dataBag->get("Facebook")
            ]
        ]
    ], $salesChannelContext->getContext());
}
if ($dataBag->has('twitter')) {
    $this->merchantRepository->update([
        [
            'id' => $merchant->getId(),
            "customFields" => [
                "twitter" => $dataBag->get("twitter")
            ]
        ]
    ], $salesChannelContext->getContext());
}
      if ($dataBag->has('youtube')) {
    $this->merchantRepository->update([
        [
            'id' => $merchant->getId(),
            "customFields" => [
                "youtube" => $dataBag->get("youtube")
            ]
        ]
    ], $salesChannelContext->getContext());
}
            if ($dataBag->has('Instagram')) {
    $this->merchantRepository->update([
        [
            'id' => $merchant->getId(),
            "customFields" => [
                "Instagram" => $dataBag->get("Instagram")
            ]
        ]
    ], $salesChannelContext->getContext());
}
    ...
    $this->merchantRepository->update([
        [
            ...
            "customFields" => [
                ...
                if ($dataBag->has('tiktok')) {
    $this->merchantRepository->update([
        [
            'id' => $merchant->getId(),
            "customFields" => [
                "tiktok" => $dataBag->get("tiktok")
            ]
        ]
    ], $salesChannelContext->getContext());
}
